...
| AI 结构扫描 | 在结构树中选择厂商并输入自己的 API Key，自动获取可用模型，生成语义标签和布局风险标记 |
...
| AI 辅助 | Google、OpenAI、Claude、DeepSeek、Qwen、Kimi、GLM、豆包、千帆、MiniMax、Mistral、xAI、Groq、Together、OpenRouter、SiliconFlow（用户自带 Key，自动检索模型列表，纯前端调用） |
...
3. 在左侧切换“源码”或“结构”视图：
   - 源码视图适合直接改 HTML。
   - 结构视图适合快速定位可编辑元素。
4. 如需 AI 辅助，在结构视图选择模型厂商，填写自己的 API Key。应用会自动检索可用模型，提供模型下拉列表并显示模型家族徽标；如果厂商不支持浏览器直连模型列表，也可以选择“手动输入模型...”后点击“AI 扫描”。
5. 在中间 Canvas 中点击文本、图片或按钮元素。
6. 在右侧 Inspector 修改内容、字体、颜色、尺寸和其他样式。
7. 使用浮动工具栏进行复制样式、粘贴样式、复制、删除、移动等操作。
8. 点击“导出 HTML”，预览并下载清理后的 HTML 文件。
...
- **AI Key 安全**：GitHub Pages 版本采用用户自带 Key 的前端调用方式，模型列表检索和扫描请求都会从浏览器直接发往所选厂商，建议在厂商控制台限制 Key 的可用来源；不支持浏览器跨域调用的厂商可改用 OpenRouter 或本地代理。
...
- [ ] 组件级复制、粘贴和模板收藏
- [ ] 导出前可视化校验报告
- [ ] 可选本地 AI 代理服务，避免在浏览器里直接使用 API Key
